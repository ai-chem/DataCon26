...
> ⚠️ **Важно:** метрика рассчитывается на open access статьях. Если метрика для датасета не указана, это значит, что он не содержит open access статей. В рамках хакатона рекомендуется использовать только open access публикации.
